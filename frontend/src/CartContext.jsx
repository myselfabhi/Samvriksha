...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (cartItem) => {
    ...
    try {
      const { product, variant, size, design, color, additionalOptions, quantity } = cartItem;
      const price = size.price + additionalOptions.reduce((total, option) => total + option.price, 0);
  ...
  // Update Quantity
  const updateQuantity = async (productId, variant, size, design, color, quantity) => {
    if (!token) {
      alert("Please log in to update your cart.");
      return;
    }
  ...
  // Remove from Cart
  const removeFromCart = async (productId, variant, size, design, color) => {
    if (!token) {
      alert("Please log in to remove items from your cart.");
      return;
    }
    try {
      await axiosInstance.delete("/cart/remove", {
        data: { productId, variant, size, design, color },
      });
      fetchCart();
    } catch (error) {
      console.error("Error removing item from cart:", error.response?.data || error.message);
